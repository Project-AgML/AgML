...
def create_model(
    num_classes=1, architecture="tf_efficientdet_d4", pretrained=(False, False)
):
...
def create_model_from_pretrained(
    num_classes=1, architecture="tf_efficientdet_d4", pretrained_path=None
):
    config = get_efficientdet_config(architecture)
    config.update({"image_size": (IMAGE_SIZE, IMAGE_SIZE)})
    ...
    net.load_state_dict(torch.load(pretrained_path[0], map_location="cpu"))
    return DetBenchTrain(net, config)
...
def train_per_class(dataset, epochs, save_dir=None, overwrite=None):
    ...
    for cl in range(agml.data.source(dataset).num_classes):
        ...
        this_save_dir = os.path.join(save_dir, f"{new_loader.num_to_class[cls]}-{cls}")
        this_log_dir = this_save_dir.replace("checkpoints", "logs")

        # Set up the checkpoint saving callback.
        callbacks = [
            pl.callbacks.ModelCheckpoint(
                dirpath=this_save_dir,
                mode="min",
                filename=f"{dataset}" + "-epoch{epoch:02d}-valid_loss_{valid_loss:.2f}",
                monitor="valid_loss",
                save_top_k=3,
        ...
        # Create the loggers.
        loggers = [CSVLogger(this_log_dir), TensorBoardLogger(this_log_dir)]

        # Construct the model.
        model = EfficientDetModel(
            num_classes=1, architecture="tf_efficientdet_d4", pretrained=True
        )
        model.load_state_dict(
            torch.load(
                "/data2/amnjoshi/full_grape/checkpoints/final_model.pth",
                map_location="cpu",
            )
        )
