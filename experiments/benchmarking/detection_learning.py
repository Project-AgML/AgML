...
class EfficientDetDataModule(pl.LightningDataModule):
    ...
    def test_dataset(self) -> EfficientDetDataset:
        return EfficientDetDataset(adaptor=self.test_ds, transforms=self.val_tfms)
...
class EfficientDetModel(pl.LightningModule):
    def __init__(
        ...
        if pretrained_path is not None:
            self.model = create_model_from_pretrained(
                num_classes, architecture=architecture, pretrained_path=pretrained_path
            )
        else:
    ...
    def on_test_epoch_end(self):
        ...
        self.log(
            "test-map", map, prog_bar=True, on_epoch=True, logger=True, sync_dist=True
        )
        self.test_map.reset()
