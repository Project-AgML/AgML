...
def intersection_over_union(pred_boxes, gt_boxes):
    ...
    pred_area = abs((pred_boxes[..., 2:3] - pred_boxes[..., 0:1])
                    * (pred_boxes[..., 3:4] - pred_boxes[..., 1:2]))
    gt_area = abs((gt_boxes[..., 2:3] - gt_boxes[..., 0:1])
...
def mean_average_precision(pred_boxes,
    ...
    for c in range(num_classes):
        ...
        detections = where_pred_box[
            np.where(where_pred_box[:, 1] == c)[0]].tolist()
        ground_truths = where_true_box[
            np.where(where_true_box[:, 1] == c)[0]].tolist()
        ...
        amount_bboxes = {
             k: np.zeros(v) for k, v in zip(indices, counts)}
        ...
        for detection_idx, detection in enumerate(detections):
            ...
            ious = np.array([intersection_over_union(
