...
def show_images_and_labels(images, labels=None, info=None, shape=None, **kwargs):
    """Visualizes a set of images with their classification labels in 3 columns.
    ...
    image classification labels displayed underneath them.
    ...
    if is_array_like(labels):
        if not isinstance(labels, (list, tuple)):
            if labels.ndim == 2:  # Handle one-hot encoding
                labels = np.argmax(labels, axis=-1)

    # Ensure grid shape is 3 columns and wrap rows.
    num_images = len(images)
    num_cols = 3
    num_rows = (num_images + num_cols - 1) // num_cols  # Calculate rows based on number of images
