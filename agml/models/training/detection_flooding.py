...
from tools import gpus, checkpoint_dir, auto_move_data
...
def get_data(combo_type, dataset):
    """Gets data for the specified combo."""
    def target(annotation):
        annotation['category_id'][np.where(annotation['category_id'] >= 2)[0]] \
            = np.ones_like(np.where(annotation['category_id'] >= 2)[0]) * 2
        return annotation

    # Construct the loader and get the number of classes.
...
def train(dataset, epochs, save_dir = None, combo_type = None,
    ...
    msg = f"Training combo ({combo_type})!"
    print("\n" + "=" * len(msg) + "\n" + msg + "\n" + "=" * len(msg) + "\n")
