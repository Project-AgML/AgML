...
class HeliosDataGenerator(AgMLSerializable):
    ...
    def _convert_options_to_xml(self):
        ...
        if (
            self._generation_options.simulation_type == SimulationType.LiDAR
        ...
        if (
            self._generation_options.simulation_type == SimulationType.RGB
            or self._generation_options.simulation_type == SimulationType.Both
        ):
        ...
        if os.path.exists(output_path) and os.path.isdir(output_path):
            if any(os.scandir(output_path)):
                if clear_existing_files:
                    ...
                    raise OSError(
                        f"The provided directory for generation, "
    ...
    def _write_config(self, *, cfg_file, xml_file):
        ...
        cfg = (
            f"{self._generation_options.num_images}\n"
        ...
        *,
        ...
        if process.returncode != 0:
            if process.returncode == -11:
                raise OSError(
                    f"Encountered an error when generating synthetic "
