...
from agml.train.coco_utils import get_coco_api_from_dataset
from agml.train.coco_eval import CocoEvaluator
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq):
    ...
    data_loader.dataset.dataset.train=True
...
def validate(model, data_loader, device, best_val_loss):
    ...
    #header = 'Epoch: [{}]'.format(epoch)
    total_val_loss=0
    data_loader.dataset.dataset.train=False # a way to turn off augmentation during validation when you have use torch random splitting
    ...
    if total_val_loss<best_val_loss:
        ...
        #torch.save(model, 'best_models/no_del.pth')
